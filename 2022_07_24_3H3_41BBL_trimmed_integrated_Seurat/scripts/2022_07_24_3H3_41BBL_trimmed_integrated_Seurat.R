...
directories = directories[!str_starts(directories, pattern = fixed("Naive"))]
print(directories)
...
create_data_list <- function(dat_list, project, data_dir){
    ...
    dat_list[[project]][['Experiment']] <- gsub("_.*", "", project)
    return(dat_list)
}

for(i in 1:length(directories)) {
  dat_list <- create_data_list(dat_list, directories[i], file.path(folder_path, directories[i], "outs", "filtered_feature_bc_matrix"))
}
...
dat <- merge(
	...
	y = c(
		...
		dat_list[[directories[5]]],
		dat_list[[directories[6]]],
		dat_list[[directories[7]]],
		dat_list[[directories[8]]],
		dat_list[[directories[9]]],
		dat_list[[directories[10]]],
		dat_list[[directories[11]]],
		dat_list[[directories[12]]],
		dat_list[[directories[13]]],
		dat_list[[directories[14]]],
		dat_list[[directories[15]]]
	), 
...
for (i in 1:length(dat.list)) {
  if (names(dat.list)[i] == "Naive") {
    dat.list[[i]] <- subset(dat.list[[i]], subset = nFeature_RNA > 100 & nFeature_RNA < 6000 & percent.mt < 50)
  } else {
    dat.list[[i]] <- subset(dat.list[[i]], subset = nFeature_RNA > 100 & nFeature_RNA < 7500 & percent.mt < 50)
  }
}
